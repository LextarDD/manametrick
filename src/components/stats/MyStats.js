...
/* ── Custom dropdown — usa position:fixed para escapar overflow:hidden ── */
  ...
  return (
    <div style={{ position: 'relative', minWidth: 150 }}>
      ...
      >
        ...
        <span style={{ fontSize: 10, color: '#6a6a9a', transition: 'transform .2s', display: 'inline-block', transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}>▾</span>
      </button>
      {open && (
        <div
          ref={panelRef}
          ...
          {options.map(opt => (
            <div
              ...
              style={{
                ...
                color: opt.value === value ? '#a89fff' : '#c8beff',
                background: opt.value === value ? 'rgba(108,87,255,.2)' : 'transparent',
                transition: 'background .12s',
              }}
              onMouseEnter={e => { if (opt.value !== value) e.currentTarget.style.background = 'rgba(255,255,255,.05)'; }}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <div style={{ flex: 1, height: 8, background: 'rgba(255,255,255,0.08)', borderRadius: 4, overflow: 'hidden' }}>
        <div style={{ width: `${winrate}%`, height: '100%', background: color, borderRadius: 4, transition: 'width 0.5s ease' }} />
      </div>
      <span style={{ fontFamily: 'monospace', fontSize: 13, color, fontWeight: 700, minWidth: 38, textAlign: 'right' }}>{winrate}%</span>
    </div>
  );
}
...
function StatRow({ item, onClick, index }) {
  return (
    <div
      onClick={onClick}
      style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 180px', gap: 12, alignItems: 'center', padding: '14px 20px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', cursor: 'pointer', transition: 'background 0.15s, border-color 0.15s' }}
    ...
    >
      <span style={{ fontWeight: 600, fontSize: 14, color: '#e2e8f0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</span>
      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', textAlign: 'center', fontFamily: 'monospace' }}>
        <span style={{ color: '#4ade80' }}>{item.wins}W</span>{' / '}<span style={{ color: '#f87171' }}>{item.losses}L</span>
      </span>
      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', textAlign: 'center', fontFamily: 'monospace' }}>{item.total} gg</span>
...
export default function MyStats({ games, decks, archetypes, onCreateTournament, user }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        ...
        {onCreateTournament && (
          <button onClick={onCreateTournament}
            style={{ padding: '10px 20px', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}
      ...
      {/* Filters */}
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', padding: '16px 20px', background: 'rgba(255,255,255,0.03)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
        <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Filtros</span>
...
function ListHeader() {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 180px', gap: 12, padding: '6px 20px' }}>
      {['Nombre', 'W / L', 'Partidas', 'Winrate'].map(h => (
        <span key={h} style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'rgba(255,255,255,0.3)', textAlign: h === 'Nombre' ? 'left' : 'center' }}>{h}</span>
      ))}
    </div>
  );
}
...
function Empty({ text }) {
  return <div style={{ textAlign: 'center', padding: '40px 20px', color: 'rgba(255,255,255,0.3)', fontSize: 14 }}>{text}</div>;
}
