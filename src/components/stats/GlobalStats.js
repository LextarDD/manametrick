...
import GlobalMatchupDetails from './GlobalMatchupDetails';
...
export default function GlobalStats() {
  ...
  const [selectedMatchup, setSelectedMatchup]             = useState(null); // { playerArch, oppArch }
  const [showTop8Only, setShowTop8Only]                   = useState(false);

  const archetypeList = archetypeStats.map(a => a.archetype);
  const matrixMap     = normalizeMatrix(matchupMatrix);
  ...
  const top8      = [...archetypeStats].sort((a, b) => b.total - a.total).slice(0, 8);
  ...
  const handleMatrixClick = (playerArch, oppArch) => {
    setSelectedMatchup({ playerArch, oppArch });
  };
      ...
      <section style={styles.section}>
        <div style={styles.sectionHeader}>
          ...
          <div style={styles.toggleGroup}>
            <button style={{ ...styles.toggleBtn, ...(showTop8Only ? {} : styles.toggleActive) }} onClick={() => setShowTop8Only(false)}>Todos</button>
            <button style={{ ...styles.toggleBtn, ...(showTop8Only ? styles.toggleActive : {}) }} onClick={() => setShowTop8Only(true)}>Top 8</button>
          </div>
        </div>
        ...
        {archetypeStats.length === 0 ? (
          ...
          <div style={styles.tableWrapper}>
            <table style={styles.table}>
              ...
              <tbody>
                {displayList.map((a, i) => (
                  <tr key={a.archetype} style={styles.tr}
                    onMouseEnter={e => e.currentTarget.style.background = '#1e293b'}
                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                  >
                    <td style={{ ...styles.td, color: '#64748b', width: 36 }}>{i + 1}</td>
                    <td style={styles.td}>
                      <Link to={`/archetype/${encodeURIComponent(a.archetype)}`} style={styles.archetypeLink}>{a.archetype}</Link>
                      <button style={styles.detailBtn} onClick={() => setSelectedArchetype(a.archetype)} title="Ver detalle en modal">↗</button>
                    </td>
                    ...
                    <td style={{ ...styles.td, textAlign: 'center', fontWeight: 700, color: winrateColor(a.winrate) }}>{a.winrate}%</td>
                    <td style={styles.td}>
      ...
      {archetypeList.length > 1 && (
        <section style={styles.section}>
          ...
          <MatchupMatrix
            ...
            onSelectArchetype={handleMatrixClick}
          />
        </section>
      )}
