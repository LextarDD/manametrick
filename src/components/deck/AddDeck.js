...
// Detecta cualquier variante de separador de sideboard
const isSideboardSeparator = (line) =>
  /^sideboard[:\s]*$/i.test(line.trim());
...
const AddDeck = ({ userId, archetypes, onDeckAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mainCards.length === 0) { setFormError('No se encontraron cartas válidas en el formato "4 Lightning Bolt".'); return; }
          ...
          <form onSubmit={handleSubmit} className="deck-form">
            ...
            <div className="form-group">
              ...
              <span className="form-hint">
                Una carta por línea: <code>4 Lightning Bolt</code>.
                Escribe <code>Sideboard</code> o <code>SIDEBOARD:</code> para separar el banquillo.
              </span>
            </div>
            {errors.length > 0 && (
